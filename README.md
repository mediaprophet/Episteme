...
**Semantic Enforcement:** This project includes `semantic-dictionary.json` to prevent AI agents from using vague US-tech buzzwords (like "data ownership" or "users as assets"). Instead, agents are able to be forced to use precise vocab (as may even be defined by a URI / declarative graph) as to enable improved support human rights and legal terminology (like "Agency", "Dignity", and "Personhood", or en-AU 'thongs', rather than en-US 'thongs'). The semantic dictionary is designed to adapt dynamically to the nomenclature of your selected project configuration (e.g., `solid` nomenclature vs. `webizen` nomenclature). 
